...
export default function App() {
  ...
  const [connectionMode, setConnectionMode] = useState<"demo" | "oauth" | "gas">(
    () => (localStorage.getItem("koperjemaah_gas_url") ? "gas" : "demo")
  );
  ...
  const groupStats = () => {
    ...
    const uniqueCheckedInGroup = checkedPassports.size;
    ...
    listToUse.forEach(p => {
      ...
      if (checkedPassports.has(p.paspor)) {
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="text-left flex-grow">
              <h2 className="text-sm font-bold font-display text-slate-900 flex items-center gap-2">
                <Database className="h-4.5 w-4.5 text-emerald-600" /> Konfigurasi Sumber Data Utama (Google Sheets via GAS)
              </h2>
              <p className="text-xs text-slate-500 mt-1">
                Koneksikan asisten lapangan dengan database Google Sheets Anda. Masukkan link hasil deploy Google Apps Script (GAS) Web App di bawah ini untuk sinkronisasi otomatis.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 bg-slate-100/80 px-4 py-2 rounded-xl border border-slate-200/50">
              ...
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                ...
                <span className={`h-1.5 w-1.5 rounded-full ${gasUrl ? "bg-emerald-500" : "bg-amber-500"}`}></span>
                {gasUrl ? "Terhubung GAS" : "Mode Demo Offline"}
              </span>
            </div>
          </div>
                      ...
                      >
                          ...
                          <div className="mb-5 flex items-center gap-4 border-b pb-5 border-slate-100">
                            ...
                            <div className="text-left">
                              ...
                              <span className={`inline-block mt-1.5 px-2.5 py-0.5 text-[9px] font-bold rounded-full ${
                                foundInManifest 
                      ...
                      <div className="mt-5 p-4 bg-amber-50/50 border border-amber-100/80 rounded-xl">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(manifest.filter(p => p.grup === activeGroup).length > 0 
                            ...
                            .filter(p => !logs.some(l => l.grup === activeGroup && l.pos === activeStation && l.paspor === p.paspor))
                            .map((p, pIdx) => (
                              <span 
                                key={pIdx} 
                        ...
                        {(() => {
                          const filtered = logs
                            ...
                            .filter(l => {
                              ...
                              const pilgrim = manifest.find(p => p.paspor === l.paspor) || mockManifest.find(p => p.paspor === l.paspor);
